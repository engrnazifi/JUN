...
CREATE TABLE IF NOT EXISTS how_to_buy (
    ...
    english_text TEXT,
    media_file_id TEXT,
    media_type TEXT,        -- video | document | photo
...
def howto_update_flow(m):
    ...
    if stage == "media":
        ...
        last_version = conn.execute(
            "SELECT MAX(version) FROM how_to_buy"
        ).fetchone()[0] or 0
...
@bot.message_handler(commands=["ourusers"])
def our_users_cmd(m):
    # 🔐 ADMIN ONLY
    if m.from_user.id != ADMIN_ID:
        return
    ...
    ADMIN_PAGES[m.from_user.id] = {"page": 0}
...
👥 <b>Our visited users</b>
Total: <b>{total}</b>

🟢 <b>Active users</b>: <b>{active}</b>
""",
        parse_mode="HTML",
        reply_markup=kb
    )
@bot.callback_query_handler(func=lambda c: c.data.startswith("buyers:"))
def buyers_pages(c):
    # 🔐 ADMIN ONLY
    ...
    # ❌ Kar a bari page ya zama negative
    if page < 0:
        bot.answer_callback_query(c.id)
        return
    ...
    rows = conn.execute(
        ...
        SELECT
            ...
            vu.first_name,
            vu.last_name,
            COUNT(um.id) AS total_purchase
        FROM user_movies um
        JOIN visited_users vu ON vu.user_id = um.user_id
        WHERE vu.user_id != ?            -- ❌ CIRE ADMIN
        GROUP BY um.user_id
    ...
    # ❌ Idan babu wani user
    if not rows:
        bot.answer_callback_query(c.id, "❌ Babu karin users.")
        return

    # ================= TEXT =================
    text = "<b>🟢 Active Buyers</b>\n\n"
    ...
    for r in rows:
        ...
        text += (
            ...
            f"📦 Total purchase: <b>{r['total_purchase']}</b>\n"
            f"Status: 🟢 Active\n\n"
        )
...
def getid_command(message):
    ...
    if row:
        bot.reply_to(
            message,
            f"Kamar yadda ka bukata ga ID ɗin fim din <b>{row['title']}</b>: <code>{row['id']}</code>",
            parse_mode="HTML"
        )
        return
    ...
    if len(rows) == 1:
        ...
        bot.reply_to(
            message,
            f"Kamar yadda ka bukata ga ID ɗin fim din da kake nema <b>{r['title']}</b>: <code>{r['id']}</code>",
            parse_mode="HTML"
        )
        return
...
def sendmovie_cmd(m):
    ...
    if ids:
        ...
        not_found_ids = [str(i) for i in ids if i not in [
...
def build_paid_orders_view(uid, page):  
    ...
    total = conn.execute(  
        "SELECT COUNT(*) FROM orders WHERE user_id=? AND paid=1",  
        (uid,)  
    ).fetchone()[0]  
    ...
    for oid, count, amount, title, gk_count, base_price, group_key in rows:  
        ...
        row = conn.execute(
            "SELECT downloads FROM order_downloads WHERE order_id=? AND user_id=?",
            (oid, uid)
        ).fetchone()

        used = row[0] if row else 0
        remain = MAX_DOWNLOADS - used
        ...
        if remain > 0:
            text += f"• {short} — ✅ Paid (Downloads left: {remain})\n"
            kb.row(  
                InlineKeyboardButton(  
                    "⬇️ Download",  
                    callback_data=f"deliver:{oid}"  
                )  
            )
        else:
            text += f"• {short} — ❌ Limit reached\n"
            kb.row(
                InlineKeyboardButton(
                    "🛒 BUY AGAIN",
                    callback_data=f"buy_again:{oid}"
                )
            )
...
def buyd_deeplink_handler(msg):
    ...
    old = conn.execute(
        ...
        FROM orders o
        JOIN order_items oi ON oi.order_id = o.id
        WHERE o.user_id=? AND o.paid=0 AND oi.item_id=?
        LIMIT 1
        """,
        (uid, item_id)
    ).fetchone()
...
def buy_again_handler(c):
    ...
    try:
        ...
        bot.send_message(uid, "❌ Invalid order reference.")
        return
    ...
    rows = conn.execute(
        ...
        LEFT JOIN items i ON i.id = oi.item_id
        WHERE o.id=? AND o.user_id=? AND o.paid=1
        """,
        (old_order_id, uid)
    ).fetchall()

    if not rows:
        bot.send_message(uid, "❌ Ba a samu wannan order ba.")
        return
    ...
    if not items:
        bot.send_message(uid, "❌ Wannan order babu item mai file.")
        return

    # ===============================
    # TOTAL PRICE (GROUP SAFE)
    # ===============================
    group_keys = conn.execute(
        """
        SELECT COUNT(DISTINCT i.group_key) AS gk
        FROM order_items oi
        LEFT JOIN items i ON i.id = oi.item_id
        WHERE oi.order_id=?
        """,
    ...
    if len(items) > 1 and group_keys == 1:
        total = int(items[0]["price"] or 0)
    else:
        total = sum(int(i["price"] or 0) for i in items)

    # ===============================
    # CREATE NEW ORDER (ALWAYS NEW)
    # ===============================
    ...
    # PAYMENT
    # ===============================
    title = " / ".join({i["title"] for i in items if i["title"]})

    pay_url = create_flutterwave_payment(uid, new_order_id, total, title)
    if not pay_url:
        bot.send_message(uid, "❌ Payment error.")
        return
    ...
    # ===============================
    # FINAL MESSAGE (FORMAT IRIN NAKA)
    # ===============================
    bot.send_message(
        uid,
        f"""🧾 <b>CHECKOUT</b>
...
def buygroup_handler(c):
    ...
    if owned:
        ...
        bot.send_message(
            uid,
            "✅ <b>Ka riga ka mallaki wani fim daga wannan group.</b>",
            parse_mode="HTML",
            reply_markup=kb
        )
        return
    ...
    # ===============================
    # 🛑 KARIYA 2: UNPAID ORDER (ITEM LEVEL)
    # ===============================
    old = conn.execute(
        f"""
        SELECT o.id, o.amount
    ...
    # ===============================
    # 🧪 DEBUG MESSAGE (GYARA A NAN KAWAI)
    # ===============================
    if len(items) > 1:
        dbg = (
            "🤩 <b>GROUP ORDER CREATED</b>\n\n"
            f"📦 Episodes: {len(items)}"
        )
    else:
        dbg = "🤩 <b>ORDER CREATED</b>\n\n"
        dbg += f"• {items[0]['title']}"
...
def admin_sendall_cmd(m):
    ...
    for item_id in item_ids:
        ...
        conn.execute(
            """
            INSERT OR IGNORE INTO user_movies (user_id, item_id, order_id)
            VALUES (?, ?, ?)
            """,
...
def pay_all_unpaid(call):
    ...
    # 1️⃣ DAUKO DUK UNPAID ITEMS (DA SUNA + GROUP KEY)
    rows = conn.execute(
        """
        SELECT
            oi.item_id,
    ...
    total_amount = sum(int(r["price"]) for r in rows)
    if total_amount <= 0:
        bot.answer_callback_query(call.id, "❌ Amount error")
        return

    # 🛑 KARIYA: IDAN AKWAI PAY-ALL UNPAID TUNI
    old = conn.execute(
        """
        SELECT id, amount
        FROM orders
    ...
    if old:
        ...
        # 4️⃣ TSAFTA
        conn.execute(
            "DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE user_id=? AND paid=0 AND id!=?)",
            (user_id, new_order_id)
        )
        conn.execute(
            "DELETE FROM orders WHERE user_id=? AND paid=0 AND id!=?",
            (user_id, new_order_id)
        )
    ...
    for r in rows:
        if r["group_key"]:
            if r["group_key"] not in grouped:
                grouped[r["group_key"]] = {
                    "title": r["item_title"],
                    "count": 1
                }
            else:
                grouped[r["group_key"]]["count"] += 1
        else:
            dbg += f"• {r['item_title']}\n"

    # nuna series da episode count
    for g in grouped.values():
        dbg += (
            f"• {g['title']}\n"
            f"📦 Episodes: {g['count']}\n"
...
def buy_all_handler(c):
    ...
    if not items:
        bot.answer_callback_query(c.id, "❌ No item mai delivery.")
        return
...
def _create_and_send_buyall(uid, items, c):
    ...
    if owned:
        ...
        bot.send_message(
            uid,
            "✅ <b>Ka riga ka mallaki wani fim daga ciki.</b>",
            parse_mode="HTML",
            reply_markup=kb
        )
        bot.answer_callback_query(c.id)
    ...
    if data.startswith("resend_one:"):
        ...
        if used >= 10:
            ...
            kb.add(
                InlineKeyboardButton(
                    "🔁 Buy again",
                    callback_data=f"buy_again:{item_id}"
                )
            )
...
# ================= 🔁 BUY AGAIN (ANTI DUP ORDER) =================
    if data.startswith("buy_again:"):
        try:
            item_id = int(data.split(":", 1)[1])
        except:
        ...
        # 🛑 KAR A KIRKIRI ORDER IRI 1 SAU BIYU
        old = conn.execute("""
            SELECT o.id, o.amount
            FROM orders o
            JOIN order_items oi ON oi.order_id=o.id
        ...
        if old:
            order_id = old["id"]
            total = old["amount"]
            debug = "♻️ <b>UNPAID ORDER REUSED</b>"
        else:
            item = conn.execute(
                "SELECT title, price, file_id FROM items WHERE id=?",
                (item_id,)
            ).fetchone()

            if not item or not item["file_id"]:
                bot.answer_callback_query(c.id, "❌ Item ba ya nan.")
                return

            order_id = str(uuid.uuid4())
            total = int(item["price"] or 0)

            conn.execute(
                "INSERT INTO orders (id, user_id, amount, paid) VALUES (?, ?, ?, 0)",
                (order_id, uid, total)
            )
            conn.execute(
                """
                ...
                VALUES (?, ?, ?, ?)
                """,
                (order_id, item_id, item["file_id"], total)
            )
            conn.commit()
            debug = "🆕 <b>NEW ORDER CREATED</b>"
...
⚠️ <b>Gargadi:</b>
Da fatan za ka <b>ajiye wannan Order ID sosai</b>.
Idan wata matsala ta faru (biya ko karɓa),
<b>ka tuntubi admin tare da Order ID ɗinka</b>.""",
            parse_mode="HTML"
        )

        pay_url = create_flutterwave_payment(uid, order_id, total, "Buy again")
        if not pay_url:
            bot.send_message(uid, "❌ Payment error.")
            return
